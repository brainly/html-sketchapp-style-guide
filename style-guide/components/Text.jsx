...
import Text, {
  ...
  TEXT_COLOR,
  TEXT_WEIGHT,
} from 'style-guide/src/components/text/Text';
import Headline, {
  HEADLINE_SIZE,
  HEADLINE_COLOR,
} from 'style-guide/src/components/text/Headline';
import Link, {
  LINK_SIZE,
  LINK_COLOR,
  LINK_WEIGHT,
} from 'style-guide/src/components/text/Link';
import TextBit, {
  TEXT_BIT_SIZE,
  TEXT_BIT_COLOR,
} from 'style-guide/src/components/text/TextBit';
...
const TextPage = () => {
    ...
    getValues(LINK_COLOR, false).forEach(color => {
      [true, false].forEach(underlined => {
        getValues(linkWeight, false).forEach(weight => {
          linkVariations.push(
            <div
              title={`Link/${size || 'normal'}/${color || 'normal'}/${weight ||
                'normal'}/${underlined ? 'underlined' : 'normal'}`}
            >
              <Link color={color} size={size} underlined weight={weight}>
  ...
  getValues(textSizes, false).forEach(size => {
    getValues(TEXT_COLOR, false).forEach(color => {
      getValues(textWeight, false).forEach(weight => {
        textVariations.push(
          <div
            title={`Text/${size || 'normal'}/${color || 'normal'}/${weight ||
              'normal'}`}
          >
    ...
    getValues(HEADLINE_COLOR).forEach(color => {
      [true, false].forEach(extraBold => {
        headlineVariations.push(
          <div
            title={`Headline/${size || 'normal'}/${color || 'normal'}/${
              extraBold ? 'black' : 'bold'
            }`}
          >
            <Headline color={color} size={size} extraBold={extraBold}>
              Headline: {size} {color}
            </Headline>
          </div>
        );
  ...
  getValues(TEXT_BIT_SIZE).forEach(size => {
    getValues(TEXT_BIT_COLOR).forEach(color => {
      ...
      textBitVariations.push(
        <div title={`TextBit/${size || 'normal'}/${color || 'normal'}`}>
          <TextBit color={color} size={size}>
            TextBit: {size} {color}
          </TextBit>
        </div>
      );
