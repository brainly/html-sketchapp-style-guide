  ...
  getValues(SIZE, false).forEach(size => {
    getValues(COLOR, false).forEach(color => {
      [false, true].forEach(withRoundButton => {
        const name = `Search/${size}/${color}/${
          withRoundButton ? 'round button' : '_default_'
        }`;

        const searchSizeClass =
          size === 'large' ? `sg-search sg-search--${size}` : 'sg-search';
        const searchInputColorClass =
          color === 'white' ? `sg-input--${color}` : '';
        const searchInputClass = `sg-input sg-input--${size} ${searchInputColorClass} sg-input--with-icon sg-search__input`;
          ...
          <div title={name} className="inline-item">
            <div className={searchSizeClass}>
              <div className={searchInputClass}>
                ...
                <div className={`sg-text ${placeholderSize}`}>
